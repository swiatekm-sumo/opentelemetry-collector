...
// AttributeValueSlice logically represents a slice of AnyValue.
//
// This is a reference type. If passed by value and callee modifies it, the
// caller will see the modification.
//
...
//   }
func (es AttributeValueSlice) At(ix int) AnyValue {
	return newAnyValue(&(*es.orig)[ix])
}
...
func (es AttributeValueSlice) CopyTo(dest AttributeValueSlice) {
	...
	for i := range *es.orig {
		newAnyValue(&(*es.orig)[i]).CopyTo(newAnyValue(&(*dest.orig)[i]))
	}
}
...
// AppendEmpty will append to the end of the slice an empty AnyValue.
// It returns the newly added AnyValue.
func (es AttributeValueSlice) AppendEmpty() AnyValue {
...
func (es AttributeValueSlice) RemoveIf(f func(AnyValue) bool) {
	newLen := 0
